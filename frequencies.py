...
class frequencies:
    def __init__(
       ...
       # filename =
        ...
        self.csvData = [eachLine.split(
            ',') for eachLine in self.csvData if len(eachLine) > 0]
            ...
            while(length > 50):
                if times == 0:
                    print(stringFormat.format(
                        name=self.attributes[ind], posValues=posVal[printFrom:printTo], frequencies=freq[printFrom:printTo]))

                else:

                    print(stringFormat.format(name=" ",
                                              posValues=posVal[printFrom:printTo],
                                              frequencies=freq[printFrom:printTo]))
